...
GUI는 실행되면 자동으로 감시를 시작합니다. `Stop`으로 중지하고 `Start`로 다시 시작할 수 있으며, 클릭 이벤트와 진단 출력은 하단 로그 영역에 표시됩니다. `Prefer`에서 `Always` 또는 `Once`를 고르면 `항상 허용`과 `한 번만 허용`이 함께 있을 때 어느 버튼을 먼저 누를지 선택할 수 있습니다.
...
기본값은 `claude.exe` 창에서만 동작합니다. 버튼 문구가 다르면 직접 지정할 수 있습니다. Claude Desktop은 버튼명을 `항상 허용 2` 또는 `한 번만 허용 2 Ctrl +Enter`처럼 단축키 번호와 함께 노출할 수 있어서, 이 도구는 지정한 버튼 문구와 정확히 같거나 그 문구로 시작하는 버튼을 클릭합니다. `항상 허용`과 `한 번만 허용`이 둘 다 있으면 기본값은 `항상 허용` 우선이며, `-Prefer Once` 또는 GUI의 `Prefer: Once`로 바꿀 수 있습니다. 키보드로 `2`를 입력하는 방식은 사용하지 않습니다.
