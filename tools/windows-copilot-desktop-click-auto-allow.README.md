# Windows Copilot Desktop Click Auto Allow

Windows desktop-click helper for visible GitHub Copilot permission prompts in
VS Code / Cursor.

This is not a CLI wrapper. It watches trusted editor windows and clicks known
approval buttons such as `Allow` after safety checks.
...
```bat
tools\windows-copilot-desktop-click-auto-allow-console.exe -Once
```

Disable mouse fallback:

```bat
tools\windows-copilot-desktop-click-auto-allow-console.exe -NoMouseFallback
```
...
```bat
tools\windows-copilot-desktop-click-auto-allow-console.exe -AllowCustomTarget -WindowTitleRegex "copilot|code" -ProcessNameRegex "code|cursor" -TargetPathRegex ""
```

Changing approval button labels now requires explicit opt-in:

```bat
tools\windows-copilot-desktop-click-auto-allow-console.exe -AllowCustomButtonText -ButtonText "Allow,Continue"
```

Disabling sensitive prompt blocking requires explicit opt-in:

```bat
tools\windows-copilot-desktop-click-auto-allow-console.exe -AllowSensitivePrompt
```
